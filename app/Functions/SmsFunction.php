...
class SmsFunction
{
    ...
    public static function validate_header($header, $requestor_no)
    {
        ...
        //     return $invalid_date;
        // }
        $to_not_exist = SmsFunction::to_not_exist($header, $requestor_no);
        if ($to_not_exist) {
            return $to_not_exist;
        }
        $from_not_exist = SmsFunction::from_not_exist($header, $requestor_no);
        if ($from_not_exist) {
            return $from_not_exist;
        }
    ...
    public static function missing_code($data, $type, $requestor_no)
    {
        if ($type == "header") {
            ...
            $type = "Missing from location code";
    ...
    public static function missing_to($header, $requestor_no)
    {
        $order_no = explode("-", $header)[1];
        $type = "Missing to location code.";
    ...
    public static function to_not_exist($header, $requestor_no)
    {
        ...
        if (!$is_tag) {
            return "To location not tagged.";
        }
    }
    public static function from_not_exist($header, $requestor_no)
    {
        $location_code = explode("-", $header)[1];
        $store_code = explode("#", $location_code)[0];
        $is_tag = User::with("scope_order")
            ->where("mobile_no", $requestor_no)
            ->whereHas("scope_order", function ($query) use ($store_code) {
                $query->where("location_code", $store_code);
        ...
        if (!$is_tag) {
            return "From location not tagged.";
        }
    }
    ...
    public static function get_location_department()
    {
    ...
    public static function save_sms_order($header, $body, $requestor_no, $requestor_id, $keyword)
    {
        $type = "success";
        $store_code = explode("-", $header)[0];
        $to_location = explode("-", $header)[1];
        $to_location_code = explode("#", $to_location)[0];
        $store_details = User::where("mobile_no", $requestor_no)->first();
        return $location_details = SmsFunction::get_location_department();
        $keyword_details = Keyword::where("code", $keyword)->first();
        $to_department_details = Department::where("code", $to_location_code)->first();

        return $to_location_details = Location::with("departments")
            ->whereHas->whereIn("code", $to_department_details)
            ->get();

        $keyword_id = $keyword_details->id;
        $keyword_code = $keyword_details->code;
        $keyword_name = $keyword_details->name;

        $location_id = $to_department_details->sync_id;
        $location_code = $to_department_details->code;
        $location = $to_department_details->name;

        $department_id = $to_location_details->sync_id;
        $department_code = $to_location_details->code;
        $department = $to_location_details->name;
    ...
    public static function get_transaction($store_code, $order_no)
    {
